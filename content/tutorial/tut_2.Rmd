...
I thus assume that you already know what a null model is. :smile:
...
First install packages `install.packages("doParallel")`. The <i>doParallel</i> library installs both <i>parallel</i> and <i>foreach</i> packages.
...
```
This will create a <i>Parallel Socket Cluster</i> (PSOCK), which can be done in all operating systems. However, it only starts with the base packages and thus you need to export variables, functions and packages to the cluster (see further below) if you need them in your parallel code.
...
The "2" stands for the number of cores in the cluster.
